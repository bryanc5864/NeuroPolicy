...
"""M41: Multi-dataset pooled pretraining for cross-dataset LOSO — true novelty.
...
M41 pretrains a shared EEGNet encoder on pooled bci2a + bci2b + Lee2019
...
Three configurations per held-out subject:
  * `single`: EEGNet pretrained only on the dataset's training subjects
              (= the existing M8/M15 LOSO baseline).
