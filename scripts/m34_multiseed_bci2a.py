...
"""M34: Multi-seed multi-subject within-subject bci2a 4-class verification.

W6/SOTA-confirmation push. M10 reported the headline 97.4% acc / 57.5 b/m
...
one subject. M34:
  * 3 subjects: 1, 3, 7 (matching M1 sanity sweep — high/mid CSP+LDA acc).
  * 5 seeds per (subject, config).
  * 4 configs (same as M10): scalar/cvar × CMDP off/on (eps=0.10).
...
Output: experiments/m34_multiseed_bci2a/summary.json
"""
...
log = logging.getLogger("m34_multiseed_bci2a")
...
def build_buffer_for_subject(tb, device):
    """Standard within-subject buffer construction (mirrors M10/M30)."""
...
def main():
    ...
    out_dir = EXPERIMENTS_DIR / "m34_multiseed_bci2a"
    out_dir.mkdir(parents=True, exist_ok=True)
    log.info("M34: %d subjects × %d seeds × %d configs = %d agents",
