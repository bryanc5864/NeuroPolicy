# MIT License, 2026
"""NeuroPolicy with CTNet encoder on canonical bci2a 4-class.

Drop-in encoder swap from EEGNet to CTNet (Zhao et al., Scientific
...
Reports 2024) which reports 82.5% subject-specific bci2a accuracy.
commit accuracy on the easy fraction of trials.

Configs: scalar_a1 (baseline CQL) and cvar_a0.25_cmdp_eps0.10 (full).

Output: experiments/neuropolicy_canonical_bci2a_ctnet/summary.json
"""
...
log = logging.getLogger("neuropolicy_ctnet_canonical")

SUBJECTS = [1, 3, 7]
SEEDS = [0, 1, 2, 3, 4]
ENCODER = "ctnet"   # vs "eegnet"/"conformer" baselines
CONFIGS = [
...
def main():
    ...
    out_dir = EXPERIMENTS_DIR / "neuropolicy_canonical_bci2a_ctnet"
    ...
    log.info("\n\n=== CROSS-SUBJECT AGGREGATE (CTNet encoder, N=3 subj) ===")
    cs = {}
    for cfg in CONFIGS:
        name = cfg["name"]
