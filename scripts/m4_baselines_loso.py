...
"""Milestone 4: Baseline LOSO sweep on BCI-IV-2b.

Same pipeline as M8 (encoder pretrain → episode build → eval) but evaluating
non-RL baselines instead of NeuroPolicy. Apples-to-apples with M8's cql_a1 row
...
in M8's behavior policies — so eegnet_fixed and eegnet_threshold are literally
"the M8 logging-policy heuristics evaluated as final policies."

Output: experiments/m4_baselines/summary.json
"""
...
log = logging.getLogger("m4_baselines")
...
def run_loso(subjects: list[int], dataset_key: str = "bci2b",
    ...
    out_dir = EXPERIMENTS_DIR / "m4_baselines"
    ...
    for held_out in subjects:
        ...
        # --- 1) Encoder pretrain (matches M8 exactly: same seed, same hparams) ---
        ...
        # --- 5) Evaluate baselines via env (apples-to-apples with M8) ---
