...
"""M22: Slope-corrected FQE — apply M21's diagnosed correction.

M21 found V_FQE = 0.566 * V_GT + 0.696, i.e. cross-subject FQE shrinks
toward zero. This script applies the inverted correction
  V_FQE_corrected = (V_FQE - 0.696) / 0.566
and re-runs the M20 deployment evaluation. We also test a
...
close the gap-to-oracle that M20 surfaced?

Output: experiments/m22_slope_corrected/summary.json
"""
...
def main():
    out_dir = ROOT / "experiments" / "m22_slope_corrected"
    ...
    log.info("\n=== Naive policy selection BEFORE correction (M20 baseline) ===")
