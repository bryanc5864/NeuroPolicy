# MIT License, 2026
"""Compare  LaBraM LOSO vs  EEGNet LOSO on bci2a (paired test)."""
...
def main():
    eegnet = json.loads((ROOT / "experiments" / "loso_bci2a" / "summary.json").read_text())
    labram = json.loads((ROOT / "experiments" / "labram_loso_bci2a" / "summary.json").read_text())

    e_per = {r["held_out"]: r["results"]["cmdp_eps0.1"]["metrics"] for r in eegnet["per_subject"]}
    l_per = {r["held_out"]: r["results"]["labram_cmdp"]["metrics"] for r in labram["per_subject"]}
    ...
    out_path = ROOT / "experiments" / "labram_loso_bci2a" / "labram_vs_eegnet_bci2a_compare.json"
