...
ROOT = Path(__file__).resolve().parents[1]
 = ROOT / "experiments" / "baselines_loso_bci2b" / "summary.json"
 = ROOT / "experiments" / "loso_bci2b" / "summary.json"
FIG = ROOT / "figures"
FIG.mkdir(parents=True, exist_ok=True)


def load_per_subject_returns():
    d4 = json.loads(.read_text())
    d8 = json.loads(.read_text())
...
def figure_pareto(np_ret, bases, aggs):
    ...
    for name in order:
        ...
        ax.scatter(a["acc_mean"], a["itr_mean"], color=colors[name], s=200,
                   ...
                   label=pretty[name])
    np_agg = [row for row in json.loads(.read_text())["aggregate"]
