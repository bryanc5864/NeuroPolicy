# MIT License, 2026
""": Does WIS make naive cross-subject OPE deployment viable?

 found naive FQE deployment is harmful relative to default-T0:
  V_FQE_naive = -1.36 vs V_default = -1.33 on bci2b.
 proved no monotone affine FQE correction can fix this.
 found WIS dominates FQE on calibration;  confirmed at N=9.
...
i.e. whether V_WIS_naive ≥ V_default. If yes, WIS solves the 
problem that the  impossibility proved was unsolvable for FQE.

Compares 5 strategies on the  corpus (N=9 bci2b LOSO subjects):
  - oracle:      pick policy with highest V_GT (upper bound)
  - naive_FQE:   pick policy with highest V_FQE   ('s failing strategy)
  - naive_WIS:   pick policy with highest V_WIS   ('s candidate fix)
...
Output: experiments/wis_deployment/summary.json
"""
...
log = logging.getLogger("wis_deployment")
...
def main():
    out_dir = ROOT / "experiments" / "wis_deployment"
    out_dir.mkdir(parents=True, exist_ok=True)

    # Load  (full 9-subject) corpus
    m25_path = ROOT / "experiments" / "multimethod_ope_full" / "summary.json"
    if not m25_path.exists():
        log.error(" corpus not found at %s — run scripts/multimethod_ope_full.py first.",
                   m25_path)
        return
    m25 = json.loads(m25_path.read_text())
    per_subject = m25["per_subject"]
    log.info("Loaded  corpus: %d held-out subjects", len(per_subject))

    rows = evaluate_strategies(per_subject)

    log.info("\n===  deployment outcomes per subject (V_GT of selected policy) ===")
    ...
    log.info("  P(WIS naive >= default)  = %.4f  ←  impossibility resolved if ≥ 0.95",
    ...
    log.info("  P(FQE naive  <  default) = %.4f  ('s harmful claim)",
