...
"""M36: SOTA-comparison table builder for bci2a 4-class within-subject.

Aggregates:
  * M10  : single-seed cvar+cmdp on bci2a sub 3 (random split, 70/15/15)
  * M34  : multi-seed × 3 subjects × 4 configs (random split)
  * M35  : within-subject mandatory baselines (FBCSP/CSP/EEGNet-mandatory,
            canonical session-T → session-E protocol)
  * M38  : multi-seed × 3 subjects × 2 configs (canonical protocol)
...
Produces:
  * experiments/m36_sota_table/summary.json (machine-readable)
  * experiments/m36_sota_table/sota_table.md (human-readable Markdown)

The literature numbers are mean accuracy across 9 subjects (canonical
session-T → session-E split). Our numbers from M38 / M35 are mean ± std
across 5 seeds × 3 subjects on the same canonical split, directly
comparable. M10 / M34 use random splits and are flagged as "non-canonical".
"""
...
def main():
    out_dir = ROOT / "experiments" / "m36_sota_table"
    out_dir.mkdir(parents=True, exist_ok=True)

    m10 = safe_load("m10_bci2a_within/summary.json")
    m34 = safe_load("m34_multiseed_bci2a/summary.json")
    m35 = safe_load("m35_within_subject_baselines/summary.json")
    m38 = safe_load("m38_canonical_bci2a/summary.json")
    m39 = safe_load("m39_conformer_canonical_bci2a/summary.json")
    m39b = safe_load("m39b_conformer_all9/summary.json")
    ...
    # === M35: our baselines on canonical protocol ===
    ...
    # === M39 / M39b: EEG-Conformer (Song 2023) — SOTA push ===
    for tag, src in [("M39 (3 subj)", m39), ("M39b (9 subj)", m39b)]:
    ...
    # === M38: NeuroPolicy on canonical protocol ===
    if m38 is not None and "cross_subject" in m38:
        for name in m38["configs"]:
            ...
            rows.append({
                "name": f"NeuroPolicy {name} (M38, canonical, 3 subj × 5 seeds)",
                "protocol": "canonical (T→E)",
    ...
    # === M34: NeuroPolicy on random protocol (non-canonical, less comparable) ===
    if m34 is not None and "cross_subject" in m34:
        for name in m34["configs"]:
            ...
            rates = [m34["per_subject"][str(sid)]["aggregate"][name]["commit_rate_mean"]
                      for sid in m34["subjects"]]
            rows.append({
                "name": f"NeuroPolicy {name} (M34, RANDOM 70/15/15, 3 subj × 5 seeds)",
                ...
                "selective": True, "source": "M34"})

    # === M10: single-seed cvar reference ===
    if m10 is not None and "cvar_a0.25" in m10:
        t = m10["cvar_a0.25"]["test"]
        rows.append({"name": "NeuroPolicy cvar_a0.25 (M10, RANDOM 70/15/15, sub 3, seed 0)",
                      "protocol": "non-canonical random",
    ...
    out = {"rows": rows_sorted,
            ...
            "notes": ("Canonical protocol = session 0 (T, 288 trials) train+val, "
                      ...
                      "may not match the 3 subjects used in our M35/M38 — comparison is "
