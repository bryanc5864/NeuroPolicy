# MIT License, 2026
""": LaBraM foundation-model encoder on bci2a sub 3 (within-subject, 4-class).
...
Two encoders compared (everything else identical to ):
  eegnet            : EEGNet supervised-pretrained ( baseline reproduction)
...
Then NeuroPolicy (scalar CQL + CMDP eps=0.10, the  winning config) is
trained on each encoder's features and evaluated on the held-out test
split.

Output: experiments/labram_bci2a_within/summary.json
"""
...
log = logging.getLogger("labram_bci2a_within")
...
def run_one(encoder, tb, tr_idx, va_idx, te_idx, device, n_classes,
    ...
    # NeuroPolicy: scalar CQL + CMDP eps=0.10 ('s winning config)
...
def main():
    ...
    # 1) EEGNet baseline reproduction ( scalar+CMDP config)
    log.info("\n=== EEGNet baseline ===")
    ...
    out_dir = EXPERIMENTS_DIR / "labram_bci2a_within"
    out_dir.mkdir(parents=True, exist_ok=True)
    (out_dir / "summary.json").write_text(json.dumps(results, indent=2))
    log.info("Wrote %s", out_dir / "summary.json")

    log.info("\n===  SUMMARY (test split, scalar CQL + CMDP eps=0.10) ===")
