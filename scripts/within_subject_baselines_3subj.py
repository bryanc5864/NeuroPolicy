# MIT License, 2026
""": Within-subject SOTA baselines on bci2a 4-class — CANONICAL PROTOCOL.

For an honest head-to-head against published SOTA on bci2a 4-class,
 runs three mandatory-classification baselines (every trial forced
...
directly comparable to published SOTA numbers. Apples-to-apples vs .
  ...
  * `eegnet_mandatory` : Same EEGNet encoder pretrained as  → linear head
                         on trial-mean embedding → argmax (no defer).

Per subject:
  * 5 seeds (matches  — only the encoder/RNG seed varies; classical
    baselines have a single deterministic fit per seed).
  * Same 70/15/15 trial split as .
  * Reports mean ± std task accuracy + per-class breakdown.

Output: experiments/within_subject_baselines_3subj/summary.json
"""
...
log = logging.getLogger("m35_baselines")
...
def run_subject(sid, device):
    ...
    # Canonical protocol: session 0 = train+val, session 1 = test (matches )
...
def main():
    ...
    out_dir = EXPERIMENTS_DIR / "within_subject_baselines_3subj"
