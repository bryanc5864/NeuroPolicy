# MIT License, 2026
""": Bootstrap CIs on the meta-correlation (/) and deployment-
rule () headline statistics.

We perform paired-subject bootstrap (resample 9 subjects with replacement
per dataset, B=10000) and report 95% CIs on:
  - Pearson r(decodability, OPE r)  [/ meta-correlation]
  - Mean V_GT under naive / gated / default selection []
  - Gain V_gated - V_naive (no-regret claim)

Output: experiments/bootstrap_ci_deployment/summary.json
"""
...
log = logging.getLogger("bootstrap_ci_deployment")
...
def main():
    out_dir = ROOT / "experiments" / "bootstrap_ci_deployment"
