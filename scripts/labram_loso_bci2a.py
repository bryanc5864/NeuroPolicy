# MIT License, 2026
""": LaBraM LOSO sweep on BCI-IV-2a (4-class × foundation-model × cross-subject).

Same protocol as  (LaBraM LOSO bci2b) but on the 4-class bci2a dataset.
Tests whether the foundation-model encoder generalises to multi-class
cross-subject MI. Apples-to-apples with  (bci2a LOSO with EEGNet).
...
Output: experiments/labram_loso_bci2a/summary.json
"""
...
log = logging.getLogger("labram_loso_bci2a")
...
def main():
    ...
    out_dir = EXPERIMENTS_DIR / "labram_loso_bci2a"
    ...
    # Aggregate
    log.info("\n===  AGGREGATE LOSO bci2a (LaBraM + scalar CQL + CMDP eps=0.10) ===")
