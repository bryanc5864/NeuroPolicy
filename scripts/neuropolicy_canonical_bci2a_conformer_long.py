# MIT License, 2026
"""NeuroPolicy with Conformer + Song 2023 published training recipe (250 epochs + S&R aug).

Same Conformer architecture as neuropolicy_canonical_bci2a_conformer_long.py
...
but with Song 2023 published recipe: 250 epochs, Adam(b1=0.5), Shift&Rotate aug.
commit accuracy on the easy fraction of trials.

Configs: scalar_a1 (baseline CQL) and cvar_a0.25_cmdp_eps0.10 (full).

Output: experiments/neuropolicy_canonical_bci2a_conformer_long_long/summary.json
"""
...
from src.models.encoder import build_encoder, pretrain_encoder_conformer_recipe
...
log = logging.getLogger("neuropolicy_conformer_long_canonical")
...
def build_buffer_for_subject(tb, device, split_seed=0, encoder_name=ENCODER):
    ...
    pre_info = pretrain_encoder_conformer_recipe(
        enc, X=win_X, y=win_y, n_classes=n_classes, device=device,
        n_epochs=250, batch_size=72, lr=2e-4, weight_decay=1e-4, seed=0, val_frac=0.15,
    )
...
def main():
    ...
    out_dir = EXPERIMENTS_DIR / "neuropolicy_canonical_bci2a_conformer_long"
    ...
    log.info("\n\n=== CROSS-SUBJECT AGGREGATE (Conformer long recipe, N=3 subj) ===")
    cs = {}
    for cfg in CONFIGS:
        name = cfg["name"]
