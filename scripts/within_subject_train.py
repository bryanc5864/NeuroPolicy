# MIT License, 2026
"""verification: train scalar CQL on bci2b sub 4 and confirm
the agent improves over a random policy within a budget that fits the
laptop GPU.

Per RESEARCH_PLAN.md §6 'sanity check that the agent improves
...
def main():
    ...
    # 8) Save artefacts
    out_dir = EXPERIMENTS_DIR / "within_subject_train"
    ...
    log.info(" verification: agent return %.3f vs random %.3f (Δ=%+.3f)",
