# MIT License, 2026
""": Does WIS work as a deployment selector on a stochastic-only panel?

 found WIS-naive deployment collapses to deterministic argmax
...
We compare on the  N=9 corpus + paired bootstrap.

Output: experiments/wis_stochastic_subset/summary.json
"""
...
log = logging.getLogger("wis_stochastic_subset")
...
def main():
    out_dir = ROOT / "experiments" / "wis_stochastic_subset"
    out_dir.mkdir(parents=True, exist_ok=True)

    m25 = json.loads((ROOT / "experiments" / "multimethod_ope_full" / "summary.json").read_text())
    per_subject = m25["per_subject"]
    log.info("Loaded  corpus: %d subjects", len(per_subject))
