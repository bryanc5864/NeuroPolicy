...
"""M25: Extend M24's multi-method OPE benchmark to all 9 bci2b LOSO subjects.

M24 ran FQE/PDIS/WIS/DR on subjects {1, 4, 7} only (compute budget).
M25 finishes the sweep on the remaining {2, 3, 5, 6, 8, 9} so we have
N=9 paired (FQE_r, WIS_r) values per subject — enough for a Wilcoxon
signed-rank test on the WIS > FQE claim.

Strategy: import run_one_subject_multimethod from M24, run on the
missing 6 subjects, merge with M24's saved per_subject list, save the
N=9 summary for downstream M26 deployment validation.

Output: experiments/m25_multimethod_full/summary.json
"""
...
from m24_multimethod_ope import run_one_subject_multimethod

logging.basicConfig(level=logging.INFO,
                    format="%(asctime)s %(levelname)s %(name)s :: %(message)s")
log = logging.getLogger("m25_multimethod_full")


def main():
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    log.info("device=%s", device)
    out_dir = EXPERIMENTS_DIR / "m25_multimethod_full"
    out_dir.mkdir(parents=True, exist_ok=True)

    # Load M24 partial results
    m24_path = EXPERIMENTS_DIR / "m24_multimethod_ope" / "summary.json"
    m24 = json.loads(m24_path.read_text())
    done = {r["held_out"] for r in m24["per_subject"]}
    log.info("M24 already covered subjects: %s", sorted(done))

    target_subjects = [s for s in range(1, 10) if s not in done]
    log.info("M25 will run on subjects: %s", target_subjects)
    ...
    # === Aggregate ===
    log.info("\n=== M25 N=%d AGGREGATE: per-method calibration ===", len(per_subject))
