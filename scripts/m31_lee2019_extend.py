...
"""M31: Extend M28's 5-subject Lee2019 LOSO to 8 subjects.

W2/W11 power: M28 reported mean OPE r = +0.803 on 5 Lee2019 subjects.
M31 adds subjects 25, 30, 35 so that the third-dataset replication
has N=8 — enough for a paired-bootstrap CI on the channel-count
→ OPE-quality relationship.

Reuses M28's saved per_subject list; only the new 3 subjects are run.

Output: experiments/m31_lee2019_extend/summary.json
"""
...
log = logging.getLogger("m31_lee2019_extend")


def main():
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    log.info("device=%s", device)
    out_dir = EXPERIMENTS_DIR / "m31_lee2019_extend"
    out_dir.mkdir(parents=True, exist_ok=True)

    # Load M28 partial results
    m28_path = EXPERIMENTS_DIR / "m28_ope_loso_lee2019" / "summary.json"
    m28 = json.loads(m28_path.read_text())
    done = {r["held_out"] for r in m28["per_subject"]}
    log.info("M28 covered: %s", sorted(done))
    ...
    log.info("M31 target subjects: %s (running %s)", target_subjects, missing)

    log.info("Preprocessing Lee2019 subjects %s (all of M31 target)...", target_subjects)
    subject_tbs = {}
    ...
    # Aggregate
    log.info("\n=== M31 OPE-LOSO Lee2019 AGGREGATE (%d subjects) ===", len(per_subject))
    ...
    if len(paired_acc) >= 3:
        ...
        log.info("\n=== M31 META-CORRELATION (Lee2019 N=%d) ===", len(per_subject))
    ...
    log.info("  bci2b (3 ch, M17b, N=9): mean r = +0.171")
    log.info("  bci2a (22 ch, M19, N=9): mean r = +0.459")
    log.info("  lee2019 (62 ch, M31, N=%d): mean r = %.3f", len(rs_arr), rs_arr.mean())
