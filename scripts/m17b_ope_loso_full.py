...
"""M17b: OPE-LOSO calibration on remaining 6 bci2b subjects.

M17 covered subjects 1, 4, 7. This script covers 2, 3, 5, 6, 8, 9 to
give the full N=9 cross-subject OPE calibration sweep. Same protocol
as M17 (6-policy panel, 500-iter FQE per target).

Output appended to experiments/m17_ope_loso/summary.json (merged).
"""
...
# Reuse run_one_subject from M17
from m17_ope_loso import run_one_subject

logging.basicConfig(level=logging.INFO,
                    format="%(asctime)s %(levelname)s %(name)s :: %(message)s")
log = logging.getLogger("m17b_ope_loso_full")


def main():
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    log.info("device=%s", device)
    out_dir = EXPERIMENTS_DIR / "m17_ope_loso"
    out_dir.mkdir(parents=True, exist_ok=True)

    # Subjects already done in M17
    ...
    # Aggregate
    log.info("\n=== M17 FULL OPE-LOSO AGGREGATE (9 held-out subjects) ===")
