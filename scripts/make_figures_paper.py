...
new supplementary figures: SOTA leaderboard,  Lee2019 LOSO per-subject,
 9-subject EEG-Conformer per-subject.
"""
...
def fig_meta_correlation():
    bci2b = json.loads((EXP_DIR / "ope_loso_bci2b" / "summary.json").read_text())
    bci2a = json.loads((EXP_DIR / "ope_loso_bci2a" / "summary.json").read_text())
    bci2b_loso = json.loads((EXP_DIR / "loso_bci2b" / "summary.json").read_text())
    bci2a_loso = json.loads((EXP_DIR / "loso_bci2a" / "summary.json").read_text())
...
def fig_lee2019_loso():
    d = json.loads((EXP_DIR / "loso_classification_lee2019" / "summary.json").read_text())
...
# ============================================================
#  9-subject EEG-Conformer per-subject (window-avg vs full-trial)
# ============================================================
def fig_conformer_9subj():
    d = json.loads((EXP_DIR / "conformer_canonical_bci2a_all9" / "summary.json").read_text())
    subjects = d["subjects"]
    ...
    # Reference EEGNet from 
    eegnet_data = json.loads((EXP_DIR / "within_subject_baselines_all9" / "summary.json").read_text())
