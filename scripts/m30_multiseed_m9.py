...
"""M30: Multi-seed M9 — bootstrap CI on the 100% accuracy CVaR+CMDP headline.

W6 critique: every headline number is single-seed. M30 runs the M9
...
Same data + buffer construction as M9. Only the agent training seed
varies (also propagated to torch / numpy RNGs).

Output: experiments/m30_multiseed_m9/summary.json
"""
...
log = logging.getLogger("m30_multiseed")
...
def main():
    ...
    out_dir = EXPERIMENTS_DIR / "m30_multiseed_m9"
    out_dir.mkdir(parents=True, exist_ok=True)
    SEEDS = [0, 1, 2, 3, 4]
    log.info("Running M9 4-config ablation at %d seeds × 4 configs = %d agents",
