...
"""M12: CVaR-α sweep on bci2b sub 4 to trace the planned CVaR vs ITR Pareto.
...
Same buffer / encoder / training budget as m9_cvar_cmdp_within.py. We re-run
α=0.25 here for self-consistency on a single seed (rather than relying on
the M9 number that was on a different RNG sequence due to running multiple
configs in one script).

Output: experiments/m12_cvar_sweep/summary.json
"""
...
log = logging.getLogger("m12_cvar")
...
def main():
    ...
    out_dir = EXPERIMENTS_DIR / "m12_cvar_sweep"
