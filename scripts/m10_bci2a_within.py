...
"""M10: extend the within-subject ablation to BCI-IV-2a (4-class).

Same protocol as m9_cvar_cmdp_within.py, but on bci2a sub 3 (the strongest
M1 sanity-check subject — CSP+LDA 0.821) instead of bci2b sub 4. Tests:
  * Does the framework generalise from 2-class (bci2b) to 4-class (bci2a)?
  * Does the CVaR + CMDP combo also dominate scalar CQL on multi-class?
  * Does the OPE pipeline calibrate at the 4-class action space?
...
Output: experiments/m10_bci2a_within/summary.json
"""
...
log = logging.getLogger("m10_bci2a")
...
def main():
    ...
    log.info("Loaded bci2a sub 3: X=%s n_classes=%d (%s)",
             tb.X.shape, n_classes, tb.class_labels)
    ...
    log.info("Encoder pretrain set: windows=%s", win_X.shape)
    ...
    out_dir = EXPERIMENTS_DIR / "m10_bci2a_within"
