# MIT License, 2026
"""OPE calibration on bci2b sub 4 (REDUCED-SCOPE v2).
...
Gating metric per RESEARCH_PLAN §3.2.5 / §6 : FQE Pearson r >= 0.85 with
...
def main():
    ...
    # ----- Train base agent (10K steps, same as ) -----
    ...
    out_dir = EXPERIMENTS_DIR / "ope_calibration"
    ...
    ax.legend(loc="best")
    ax.grid(alpha=0.3)
    fig_path = FIGURES_DIR / "ope_calibration.png"
    fig.savefig(fig_path, bbox_inches="tight"); plt.close(fig)
    log.info("Wrote %s", fig_path)

    if pe_fqe[0] >= 0.85:
        log.info("✅  GATE PASSED: FQE Pearson r = %.3f >= 0.85", pe_fqe[0])
    else:
        log.warning("⚠️   GATE NOT YET MET: FQE Pearson r = %.3f < 0.85", pe_fqe[0])
