# MIT License, 2026
""": OPE-LOSO with LaBraM encoder — does foundation model improve
cross-subject OPE calibration?

Combines  (LaBraM LOSO) +  (cross-subject OPE) on a 3-subject
subset {1, 4, 8} of bci2b LOSO. Diagnostic subjects:
  - sub 1: medium, LaBraM marginal win in 
  - sub 4: strong cluster, both encoders match
  - sub 8: LaBraM rescue subject (: +16.8pp acc vs EEGNet)
...
Output: experiments/ope_loso_labram_bci2b/summary.json
"""
...
log = logging.getLogger("ope_loso_labram_bci2b")
...
def run_one_subject_labram(held_out, subject_tbs, n_classes, sfreq,
    ...
    log.info("\n===  OPE-LOSO held-out=%d (LaBraM) ===", held_out)
...
def main(target_subjects=None):
    ...
    out_dir = EXPERIMENTS_DIR / "ope_loso_labram_bci2b"
