# MIT License, 2026
""": CVaR-α sweep on bci2b sub 4 to trace the planned CVaR vs ITR Pareto.

RESEARCH_PLAN.md §4.1 Experiment 6: sweep α_CVaR ∈ {0.10, 0.25, 0.50, 1.0},
report CVaR_α(error cost) vs ITR. α=1.0 recovers risk-neutral CQL.

Same buffer / encoder / training budget as cvar_cmdp_within_subject.py. We re-run
α=0.25 here for self-consistency on a single seed (rather than relying on
the  number that was on a different RNG sequence due to running multiple
configs in one script).

Output: experiments/cvar_alpha_sweep/summary.json
"""
...
from src.utils.config import EXPERIMENTS_DIR, MDP_CFG, TRAIN_CFG, TrainConfig

logging.basicConfig(level=logging.INFO,
                    format="%(asctime)s %(levelname)s %(name)s :: %(message)s")
log = logging.getLogger("cvar_alpha_sweep")
...
def cvar_of_returns(returns: np.ndarray, alpha: float) -> float:
    """CVaR_alpha (lower-tail mean) of error cost = -returns. Returns the
    mean of the lowest-alpha-fraction of returns (= worst case for the agent)."""
    if alpha >= 1.0:
        return float(np.mean(returns))
    n = len(returns)
    k = max(1, int(np.ceil(alpha * n)))
    sorted_returns = np.sort(returns)[:k]
    return float(np.mean(sorted_returns))
...
def main():
    ...
    # Sweep α_CVaR: 0.10 (most conservative) → 1.0 (risk-neutral, scalar-equivalent)
    alphas = [0.10, 0.25, 0.50, 1.0]
    results = {
        "subject_id": int(tb.subject_id), "dataset_key": tb.dataset_key,
        "alphas": alphas, "encoder_val_acc": float(pre_info["best_val_acc"]),
        "random_val": rand_val.asdict(), "random_test": rand_test.asdict(),
    }
    points = []
    for alpha in alphas:
        log.info("=== α_CVaR = %.2f ===", alpha)
        cfg_obj = TrainConfig(**{**vars(TRAIN_CFG), "cql_alpha": 1.0,
                                  "cvar_alpha": alpha, "seed": 0})
        agent = NeuroPolicyAgent(
            state_dim=env.state_dim, n_actions=env.action_space.n,
            n_classes=n_classes, cfg=cfg_obj,
            mode="distributional", cmdp_eps=None,
        ...
        test = evaluate_policy(env, test_eps, lambda s: agent.select_action(s),
                               n_classes=n_classes)
        # CVaR_alpha of error cost (= negative return; lower is worse)
        # Equivalently: lower-tail mean of returns (worst-case for agent).
        cvar_test = cvar_of_returns(test.returns, alpha)
        log.info("  test: ret=%+.3f acc=%.3f ITR=%.2f wrong=%.3f n_commit=%d  CVaR_%.2f(test)=%+.3f  (%.1fs)",
                 test.return_mean, test.accuracy_on_commits,
                 test.information_transfer_rate, test.wrong_commit_rate,
                 test.n_commits, alpha, cvar_test, elapsed)

        points.append({
            "alpha": alpha,
            "val": val.asdict(),
            "test": test.asdict(),
            "cvar_lower_tail_test_returns": cvar_test,
            "elapsed_s": float(elapsed),
        })
    results["points"] = points

    out_dir = EXPERIMENTS_DIR / "cvar_alpha_sweep"
    ...
    # Console summary
    log.info("\n=== CVaR sweep summary ===")
    log.info("α     | test return | test acc | test ITR | test wrong | CVaR_α(returns)")
    for p in points:
        log.info("%.2f  | %+.3f      | %.3f    | %.1f     | %.3f      | %+.3f",
                 p["alpha"], p["test"]["return_mean"], p["test"]["accuracy_on_commits"],
                 p["test"]["information_transfer_rate"],
                 p["test"]["wrong_commit_rate"],
                 p["cvar_lower_tail_test_returns"])
