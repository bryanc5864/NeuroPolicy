# MIT License, 2026
""": OPE-LOSO on bci2a (4-class) — validate cross-subject OPE
deployment rule on a second dataset.

If the  finding (Pearson( acc, OPE r) = +0.83) replicates on
bci2a ( acc → bci2a OPE r), the deployment rule holds across
datasets. We run all 9 bci2a subjects with the same 6-policy panel.

Output: experiments/ope_loso_bci2a/summary.json
"""
...
# Reuse run_one_subject from  (it preprocesses encoder and uses the EEGNet pipeline)
from ope_loso_bci2b import run_one_subject

logging.basicConfig(level=logging.INFO,
                    format="%(asctime)s %(levelname)s %(name)s :: %(message)s")
log = logging.getLogger("ope_loso_bci2a")


def main():
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    log.info("device=%s", device)
    out_dir = EXPERIMENTS_DIR / "ope_loso_bci2a"
    ...
    # Aggregate + meta-correlation with 
    log.info("\n===  OPE-LOSO bci2a AGGREGATE (9 held-out subjects) ===")
    ...
    # Meta-correlation:  EEGNet bci2a acc vs  OPE r
    try:
        m15_path = ROOT / "experiments" / "loso_bci2a" / "summary.json"
        if m15_path.exists():
            ...
            if len(paired_acc) >= 3:
                ...
                log.info("\n===  META-CORRELATION ( acc vs  OPE r) ===")
                log.info("Pearson  r = %+.3f (p=%.3f)", pr, p_pr)
                log.info("Spearman ρ = %+.3f (p=%.3f)", sr, p_sr)
                # Compare to  on bci2b: Pearson +0.83 (p=0.006)
                log.info("\nDeployment rule consistency:")
                log.info("   bci2b: Pearson(decodability, OPE r) = +0.83 (p=0.006)")
                log.info("    bci2a: Pearson(decodability, OPE r) = %+.3f (p=%.3f)", pr, p_pr)
    except Exception as exc:
