# MIT License, 2026
""": OPE-LOSO on Lee2019 — third-dataset replication of the deployment rule.

W2, W11 critique: meta-correlation rule (/) shown only on two
...
Output: experiments/ope_loso_lee2019/summary.json
"""
...
log = logging.getLogger("ope_loso_lee2019")


def main():
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    log.info("device=%s", device)
    out_dir = EXPERIMENTS_DIR / "ope_loso_lee2019"
    ...
    log.info("\n===  META-CORRELATION (encoder val acc vs OPE r) ===")
    paired_acc, paired_r = [], []
    ...
    if len(paired_acc) >= 3:
        ...
        log.info("    bci2a:    Pearson(decodability, OPE r) = +0.61 (p=0.08)")
        log.info("    lee2019:  Pearson(decodability, OPE r) = %+.3f (p=%.3f)",
                 pr, p_pr)
