# MIT License, 2026
""": First multi-method OPE benchmark for BCI offline-RL.

Extends 's pipeline (cross-subject FQE on bci2b LOSO) by computing
...
per ) to keep compute manageable.

Output: experiments/multimethod_ope_3subj/summary.json
"""
...
def main():
    ...
    out_dir = EXPERIMENTS_DIR / "multimethod_ope_3subj"
    ...
    log.info("\n===  AGGREGATE: per-method calibration across %d subjects ===",
              len(per_subject))
    methods = ["v_fqe", "v_pdis", "v_wis", "v_dr"]
    agg = {}
    for m in methods:
