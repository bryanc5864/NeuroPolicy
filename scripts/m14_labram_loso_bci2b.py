...
"""M14: LaBraM LOSO sweep on bci2b (foundation-model cross-subject test).

The headline test of the LaBraM swap: does pretrained foundation-model
features beat from-scratch EEGNet on cross-subject motor-imagery
decoding? bci2b sub. 2 is BCI-illiterate per Tangermann 2012, so we
expect bimodality even with LaBraM, but the question is whether
LaBraM tightens the cluster.

Apples-to-apples comparison: same BCIEnv, same buffer construction, same
agent (scalar CQL + CMDP eps=0.10), same 10K training steps. The ONLY
difference vs M8 is the encoder.

Output: experiments/m14_labram_loso_bci2b/summary.json
"""
...
log = logging.getLogger("m14_labram_loso")
...
def finetune_labram_e2e(encoder, X, y, n_classes, device,
    ...
    """End-to-end fine-tune LaBraM with classification head.

    LOSO has 5760 training windows per held-out subject (8x within-subject),
    so fewer epochs (8 vs 15) suffice to avoid overfitting. AdamW lr=1e-4,
    gradient clip 1.0.
    """
    encoder.unfreeze()
    ...
    for ep in range(n_epochs):
        ...
        for i in range(0, len(tr_perm), batch_size):
            bi = tr_perm[shuffle[i:i + batch_size]]
            feat = encoder(X_t[bi])
            logits = head(feat)
        ...
        with torch.no_grad():
            chunks = []
            for i in range(0, len(va_perm), batch_size):
                bi = va_perm[i:i + batch_size]
                chunks.append(head(encoder(X_t[bi])))
            pred = torch.cat(chunks, 0).argmax(1)
            va = float((pred == y_t[va_perm]).float().mean())
        if va > best_va:
            best_va = va
...
pretrain_labram_head = finetune_labram_e2e  # backward-compat alias
...
def main():
    ...
    out_dir = EXPERIMENTS_DIR / "m14_labram_loso_bci2b"
    out_dir.mkdir(parents=True, exist_ok=True)
    summary_path = out_dir / "summary.json"

    log.info("Preprocessing subjects...")
    subject_tbs = {}
    for sid in subjects:
    ...
    log.info("bci2b setup: %d ch, ch_names=%s, %d samples/window @%g Hz",
             n_channels, ch_names, win_samples, sfreq)
    ...
    for held_out in subjects:
        ...
        # Build LaBraM (frozen, pretrained); head pretrained on training subjects.
        enc = build_encoder("labram", n_channels=n_channels,
                            n_samples=win_samples, sfreq=sfreq,
                            ch_names=ch_names,
                            load_pretrained=True).to(device)
        log.info("  encoder: %s, embed_dim=%d, ckpt loaded keys=%d/%d",
                 enc.spec.name, enc.spec.embed_dim,
                 enc._n_loaded, enc._n_total_ckpt)

        train_tb_list = [subject_tbs[s] for s in train_subjs]
        win_X, win_y = _windowize(train_tb_list, MDP_CFG)
        head_info = pretrain_labram_head(
            enc, win_X, win_y, n_classes, device,
            n_epochs=10, batch_size=256, lr=1e-3,
        )
        ...
        held_eps, _, _ = build_episodes_for_trial_batch(
            subject_tbs[held_out], encoder=enc, device=device,
        ...
        # Single config: scalar CQL + CMDP eps=0.10 (mirrors M8 cmdp_eps0.1)
        ...
        summary_path.write_text(json.dumps(
            {"per_subject": all_results, "subjects": subjects,
             "encoder": "labram-base-pretrained", "dataset": "bci2b"},
            indent=2))

    # Aggregate
    log.info("\n=== M14 AGGREGATE LOSO (LaBraM + scalar CQL + CMDP eps=0.10) ===")
    rets_np = np.array([r["results"]["labram_cmdp"]["metrics"]["return_mean"]
                         for r in all_results])
    accs = np.array([r["results"]["labram_cmdp"]["metrics"]["accuracy_on_commits"]
                     for r in all_results])
    itrs = np.array([r["results"]["labram_cmdp"]["metrics"]["information_transfer_rate"]
                     for r in all_results])
    wrongs = np.array([r["results"]["labram_cmdp"]["metrics"]["wrong_commit_rate"]
                       for r in all_results])
    ...
    log.info("LaBraM:  ret=%+.3f±%.2f  acc=%.3f±%.3f  ITR=%.1f±%.1f  wrong=%.3f",
             rets_np.mean(), rets_np.std(ddof=1), accs.mean(), accs.std(ddof=1),
    ...
    try:
        ...
        log.info("Paired Wilcoxon (one-sided, NP > random): p = %.3f", p_one)
    ...
    summary = {
        ...
        "aggregate": {
            "labram_cmdp": {
                "return_mean": float(rets_np.mean()),
                "return_std": float(rets_np.std(ddof=1)),
                "acc_mean": float(accs.mean()),
                "acc_std": float(accs.std(ddof=1)),
                "itr_mean": float(itrs.mean()),
                "itr_std": float(itrs.std(ddof=1)),
                "wrong_mean": float(wrongs.mean()),
            },
            "random": {
                "return_mean": float(rand_rets.mean()),
                "acc_mean": float(rand_accs.mean()),
            },
