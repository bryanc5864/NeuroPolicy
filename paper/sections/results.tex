...
($82.5\%$~\cite{Zhao2024CTNet}), and EEGEncoder
($86.5\%$~\cite{Liao2025EEGEncoder}). To test whether the gap was
purely a compute deficit, we re-ran the EEG-Conformer mandatory
classifier at the full Song 2023 budget: $2000$ epochs with S\&R
augmentation ($N_s\!=\!8$), 9 subjects $\times$ 3 seeds (per-subject
in Appendix~\ref{app:conformer_2k}). The mean
moves to $0.735\!\pm\!0.11$ --- statistically indistinguishable
from the $80$-epoch number ($0.745$, paired-subject $\Delta$
$\!=\!-0.011$) and still $\sim\!5$pp below published. The
extended-budget result is non-uniform: subjects~$1, 3, 7, 8$
improve ($+0.07$ on average) while subjects~$5, 6$ regress
($-0.07$). Strong subjects benefit from longer pretraining; hard
subjects overfit. The remaining gap to published Conformer is
\emph{recipe} (batch construction, exact augmentation schedule,
or initialisation) rather than compute, and we attribute the
$\sim\!5$pp ceiling to it.
We benchmark our selective decoder only against
\emph{reproducible} open-source baselines (EEGNet, EEG-Conformer,
CTNet --- all with public reference implementations). The higher
published numbers from EEGEncoder~\cite{Liao2025EEGEncoder}
($86.5\%$) and the two-stage Transformer
of Chaudhary~et~al.~\cite{Chaudhary2024TwoStageMI} ($88.5\%$) are
not directly comparable: neither paper releases code, EEGEncoder's
TCN dilation pattern and transformer embedding dimension are
undocumented, Chaudhary's training schedule and exact protocol
are paywalled, and both architectures are hardcoded to full
$4$-s trials (incompatible with our $1$-s growing-window selective
pipeline without surgical changes). We therefore cite their
mandatory-classifier numbers in Table~\ref{tab:sota} but do not
attempt to reimplement them as selective-decoder baselines.
