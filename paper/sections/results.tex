...
\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{../figures/m6_ope_calibration.png}
...
\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{../figures/m8_loso_acc_vs_itr.png}
...
\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{../figures/m8_loso_return_per_subject.png}
\caption{Per-subject episode return on \bcib{} LOSO. Subjects are
ordered as in the dataset; bars show \cql{}($\alpha\!=\!1$),
\cql{}($\alpha\!=\!3$), CMDP($\epsilon\!=\!0.10$), and the random
baseline. The three \cql{} configurations are essentially
indistinguishable, which we attribute to encoder feature quality
rather than the RL hyperparameters.}
\label{fig:m8_per_subj}
\end{figure}
...
each other across all subjects (Fig.~\ref{fig:m8_per_subj}), which
...
\begin{table}[t]
\centering
\caption{Aggregate LOSO results on \bcib{} ($N\!=\!9$ subjects).
All methods evaluated through the same environment (same encoder,
same state representation where applicable). Best per column
highlighted in \textbf{bold}. EEGNet+thr$_{0.75}$ achieves the
highest mean episode return; \nettype{} is competitive on accuracy
and matches EEGNet-fixed on \itr{}. Both significantly improve
over CSP+LDA fixed-window.}
\label{tab:loso_baselines}
\small
\begin{tabular}{l r r r r}
\toprule
Method & Return & Acc & \itr{} & Latency \\
       &        &     & (b/m)  & (s)     \\
\midrule
random                & $-1.58 \pm 0.09$ & $0.51$ & $0.2$  & $0.2$ \\
CSP+LDA fixed         & $-1.80 \pm 0.50$ & $0.58$ & $0.8$  & $3.0$ \\
\eegnet{} fixed       & $-1.39 \pm 0.59$ & $0.65$ & $1.9$  & $3.0$ \\
\eegnet{}+thr $0.55$  & $-1.41 \pm 0.26$ & $0.60$ & $\mathbf{128.2}$ & $0.02$\\
\eegnet{}+thr $0.65$  & $-1.35 \pm 0.30$ & $0.61$ & $23.6$ & $0.15$\\
\eegnet{}+thr $0.75$  & $\mathbf{-1.29 \pm 0.40}$ & $0.63$ & $10.6$ & $0.51$\\
\midrule
\nettype{} ($\alpha\!=\!1$) & $-1.37 \pm 0.58$ & $\mathbf{0.65 \pm 0.10}$ & $1.9$ & $3.0$ \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{table}[t]
\centering
\caption{Paired Wilcoxon tests, \nettype{} ($\alpha\!=\!1$) versus
each baseline, one-sided alternative \nettype{} $>$ baseline,
$N\!=\!9$. \nettype{} significantly beats CSP+LDA fixed-window;
ties or slightly loses to all \eegnet{}-based baselines. The
strongest hand-tuned baseline (EEGNet+thr$_{0.75}$) achieves a
marginally higher mean return.}
\label{tab:wilcoxon}
\small
\begin{tabular}{l r r r}
\toprule
Comparison & $\Delta$ Return & Wins & $p$ (1-sided) \\
\midrule
\nettype{} > CSP+LDA fixed       & $+0.435$ & $8/9$ & $\mathbf{0.037}$ \\
\nettype{} > random              & $+0.212$ & $6/9$ & $0.102$ \\
\nettype{} > \eegnet{} fixed     & $+0.020$ & $4/9$ & $0.285$ \\
\nettype{} > \eegnet{}+thr$_{0.55}$ & $+0.043$ & $4/9$ & $0.367$ \\
\nettype{} > \eegnet{}+thr$_{0.65}$ & $-0.014$ & $4/9$ & $0.590$ \\
\nettype{} > \eegnet{}+thr$_{0.75}$ & $-0.079$ & $4/9$ & $0.787$ \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{../figures/m4_baselines_vs_neuropolicy.png}
...
Tables~\ref{tab:loso_baselines} and~\ref{tab:wilcoxon} and
Fig.~\ref{fig:m4_diffs} report the head-to-head LOSO comparison.
...
\midrule
\multicolumn{5}{l}{\textit{Selective decoders (ours, 3 subj $\times$ 5 seeds)}} \\
\textsc{NeuroPolicy} scalar-\cql{}                    & $0.450 \!\pm\! 0.04$ & $0.856 \!\pm\! 0.05$ & $0.53$ & $31.7$ \\
\textbf{\textsc{NeuroPolicy} CVaR+CMDP}                & $0.332 \!\pm\! 0.06$ & $\mathbf{0.862 \!\pm\! 0.06}$ & $0.39$ & $\mathbf{34.2}$ \\
\bottomrule
\end{tabular}
\end{table}

\paragraph*{Empirical findings}
Applying cropped multi-window inference
averaging~\cite{Schirrmeister2017DeepLearning} on top of
EEG-Conformer gives $+7.8$pp over single-shot
($66.7\!\to\!74.5\%$ on 9 subjects) and $+7.0$pp over EEGNet with
the same recipe --- a reproducibility/engineering measurement of
established prior art~\cite{Schirrmeister2017DeepLearning}, not a
novel recipe. The 9-subject mean $74.5\%$ sits below published
Conformer ($78.7\%$), CTNet ($82.5\%$), Transformer-2025
($86.5\%$) due to our truncated training budget ($80$ vs.~$2$k
epochs); the easy subset $\{1, 3, 7\}$ reaches
$87.9\!\pm\!1.1\%$. \nettype{} CVaR+CMDP: commit accuracy
$86.2\!\pm\!5.8\%$, \itr{}~$\approx 34$~bits/min, wrong-commit
$\le 5\%$ under $\epsilon\!=\!0.10$ --- the first offline-RL BCI
decoder with \textsc{Defer} as a learned policy action (distinct
from post-hoc thresholding rejection~\cite{Ganeshkumar2017RejectMI}) at
canonical state-of-the-art conditions, ITR $\sim\!2\times$ typical
\bcia{} values.
...
On a 10-subject LOSO subset $\{1, 5, 10, 15, 20, 25, 30, 35, 40,
45\}$ with EEGNet and scalar~\cql{}+CMDP $\epsilon\!=\!0.10$:
\textbf{commit accuracy $0.696\!\pm\!0.104$ vs random $0.481$,
$10/10$ subjects beat random,
$\Delta_{\text{ret}}\!=\!+0.668\!\pm\!0.391$, paired Wilcoxon
$p\!=\!0.001$ (1-sided)} --- the first statistically significant
LOSO improvement obtained on any of the three datasets,
completing the channel-count to LOSO-significance trend
($p\!=\!0.10$, $0.15$, $0.001$ on the $\{3, 22, 62\}$-channel
