...
($86.5\%$~\cite{Liao2025EEGEncoder}) due to our truncated
...
($N\!=\!3721$ commit decisions). (A) Commit-time distribution: the
agent rarely commits in the first second (insufficient evidence)
or the last 0.75~s (timeout-\textsc{Abstain} dominates); median
decision time $2.25$~s, mean $2.01$~s --- $\sim\!50\%$ of full
$4$-s trial length. (B) Commit accuracy by decision-time bin: late
commits ($\ge 2$~s) cluster at $0.78$--$0.82$ accuracy while early
commits ($\le 1$~s) fall to $0.60$--$0.70$. The frontier is the
expected speed--accuracy trade-off; the agent learns to wait until
the evidence is strong enough to clear the implicit CVaR threshold.}
