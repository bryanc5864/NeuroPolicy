...
The first $N\!=\!9$ LOSO multi-method OPE benchmark for BCI
(full table: Appendix~\ref{app:multi_ope}), adding weighted
importance sampling (WIS) to the estimators of
\S\ref{sec:methods:ope}. Mean
...
Lee2019 LOSO ($N\!=\!8$ OPE subset; the classification benchmark
below uses 10 subjects): mean OPE
...
EEGEncoder \cite{Liao2025EEGEncoder}                  & $0.865$ & $0.865$ & $1.00$ & --- \\
Two-stage Transformer \cite{Chaudhary2024TwoStageMI}  & $0.885$ & $0.885$ & $1.00$ & --- \\
\midrule
...
\textbf{EEG-Conformer + window-avg (80 ep)}            & $\mathbf{0.745 \!\pm\! 0.13}$ & $\mathbf{0.745}$ & $1.00$ & --- \\
EEG-Conformer + window-avg ($2$k ep + S\&R)            & $0.735 \!\pm\! 0.11$ & $0.735$ & $1.00$ & --- \\
\midrule
...
\textsc{NeuroPolicy}+CTNet scalar-\cql{}        & $0.359 \!\pm\! 0.12$ & $0.731 \!\pm\! 0.12$ & $0.49$ & $19.0$ \\
\textsc{NeuroPolicy}+CTNet CVaR+CMDP            & $0.351 \!\pm\! 0.13$ & $0.734 \!\pm\! 0.13$ & $0.49$ & $20.0$ \\
\textsc{NeuroPolicy}+Conformer (Song 2023 recipe) scalar-\cql{} & $0.409 \!\pm\! 0.06$ & $0.795 \!\pm\! 0.04$ & $0.52$ & $30.7$ \\
\textsc{NeuroPolicy}+Conformer (Song 2023 recipe) CVaR+CMDP   & $0.311 \!\pm\! 0.05$ & $0.856 \!\pm\! 0.03$ & $0.37$ & $32.7$ \\
\midrule
\multicolumn{5}{l}{\textit{Selective decoders (ours, all 9 subj $\times$ 5 seeds, same protocol)}} \\
\textsc{NeuroPolicy}+Conformer scalar-\cql{}    & $0.336 \!\pm\! 0.07$ & $0.705 \!\pm\! 0.13$ & $0.48$ & $23.9 \!\pm\! 12.5$ \\
\textsc{NeuroPolicy}+Conformer CVaR+CMDP        & $0.216 \!\pm\! 0.06$ & $0.746 \!\pm\! 0.12$ & $0.29$ & $24.9 \!\pm\! 11.3$ \\
\bottomrule
\end{tabular}
\end{table*}

\paragraph*{Empirical findings}
Cropped multi-window averaging~\cite{Schirrmeister2017DeepLearning}
on EEG-Conformer gives $+7.8$pp over single-shot
($66.7\!\to\!74.5\%$ on 9 subjects) and $+7.0$pp over EEGNet on
the same recipe. The 9-subject mean $74.5\%$ sits below published
Conformer ($78.7\%$), CTNet ($82.5\%$~\cite{Zhao2024CTNet}), and
EEGEncoder ($86.5\%$~\cite{Liao2025EEGEncoder}). To test whether
the gap is compute, we re-ran EEG-Conformer at the full
Song~2023 budget ($2000$ epochs + shift-and-rotate (S\&R)
augmentation, 9 subj $\times$ 3
seeds; Appendix~\ref{app:conformer_2k}): mean moves to
$0.735\!\pm\!0.11$ --- statistically indistinguishable from the
$80$-epoch number ($\Delta\!=\!-0.011$), still $\sim\!5$pp
below published, with strong subjects improving and hard subjects
regressing. The remaining gap is \emph{recipe}, not compute. We
therefore benchmark our selective decoder against reproducible
open-source baselines only (EEGNet, EEG-Conformer, CTNet); the
higher published numbers from EEGEncoder
($86.5\%$~\cite{Liao2025EEGEncoder}) and the two-stage Transformer
($88.5\%$~\cite{Chaudhary2024TwoStageMI}) are closed-source and
hardcoded to full $4$-s trials (incompatible with our $1$-s
growing-window pipeline), so we cite their mandatory numbers but
do not reimplement them as selective baselines.

The selective-decoder block of Table~\ref{tab:sota} reports the
accuracy--throughput Pareto frontier on the canonical 3-subject
panel: confidence-threshold dynamic stopping at low $\tau$
achieves $\!>\!300$~b/m but at near-chance accuracy; raising
$\tau$ trades \itr{} for accuracy ($66.5$~b/m at $70\%$ with
$\tau\!=\!0.9$). \nettype{}~CVaR+CMDP sits on the high-accuracy
end: $86.2\!\pm\!5.8\%$ commit accuracy, $34$~b/m, wrong-commit
$\le 5\%$ under $\epsilon\!=\!0.10$ ($\sim\!1.7\times$ mandatory
EEGNet's $20.7$~b/m while exceeding its $0.82$ accuracy). To our
knowledge this is the first offline-RL BCI decoder with
\textsc{Defer} as a \emph{learned} action, distinct from post-hoc
thresholding~\cite{Ganeshkumar2017RejectMI}.

\paragraph*{Encoder-agnostic test: Conformer and CTNet plug-ins}
To probe whether commit accuracy is encoder-bottlenecked, we
swap the EEGNet encoder for two published transformer
architectures \emph{with no other changes} to MDP, agent, or
training recipe: EEG-Conformer~\cite{Song2023Conformer}
($6$-layer transformer, $11$ temporal tokens at $1$-s windows)
and CTNet~\cite{Zhao2024CTNet} ($6$-layer transformer,
$3$ tokens at $1$-s windows after its larger CNN pooling).
Cross-subject CVaR+CMDP commit accuracy: EEGNet $0.862$,
Conformer $\mathbf{0.876\!\pm\!0.029}$ at $\mathbf{37.0}$~b/m
(the new best Pareto point), CTNet $0.734\!\pm\!0.13$ at
$20.0$~b/m. The Conformer lift is concentrated on the
weakest-decoding subject (sub~$7$: $0.798 \to 0.897$, $+9.9$pp,
with sub~$1$ and sub~$3$ tied within noise), reproducing the
LaBraM result on \bcib{} sub~$8$
(\S\ref{sec:results:labram_loso}). The CTNet plug-in performs
the opposite manoeuvre on the same subject ($0.798 \to 0.597$,
$-20.1$pp), even though CTNet's published mandatory accuracy
($82.5\%$~\cite{Zhao2024CTNet}) is higher than Conformer's
($78.7\%$~\cite{Song2023Conformer}). The mechanism is the
$1$-s window: CTNet's CNN compresses the input to only $3$
transformer tokens, starving a $6$-layer attention stack, while
Conformer's $11$ tokens are sufficient. The selective-decoder
framework is therefore encoder-agnostic in the sense that the
agent works with any encoder, but encoder choice interacts with
window length --- token count $\times$ transformer depth is the
binding constraint at $1$-s granularity, not the agent.

\paragraph*{Full 9-subject panel: encoder-quality stratification}
The headline 3-subject panel was selected by the original
within-subject protocol on $\{1, 3, 7\}$ and turns out to coincide
with the easy-decoding end of \bcia{}. Extending the same canonical
protocol to all 9 subjects (Table~\ref{tab:sota}, ``all 9 subj''
block; per-subject in Appendix~\ref{app:9subj}) gives commit
accuracy $0.746\!\pm\!0.12$ at $24.9\!\pm\!11.3$~b/m
(CVaR+CMDP, 9 subj $\times$ 5 seeds). Per-subject results
cluster bimodally on commit accuracy: subjects $1, 3, 7, 8$ reach
$0.84$--$0.89$ at $32$--$42$~b/m, matching the 3-subject headline;
subjects $2, 5, 6, 9$ cap at $0.60$--$0.69$ at $11$--$18$~b/m
regardless of agent configuration, and their (lower) encoder
validation accuracy ($\le\!0.66$) tracks the floor. Subject~$4$
sits in the middle ($0.72$ commit acc, $0.62$ encoder val). The CMDP commit-error constraint
$\epsilon\!=\!0.10$ is satisfied on all subjects (per-seed
wrong-commit rates $0.038$--$0.135$, $\sim\!95\%$ of seeds $\le 0.10$).
This stratification is the cleanest evidence in the paper that the
selective decoder is encoder-quality-bounded: the 3-subject
headline reflects the ceiling the framework can reach when the
encoder is good, and the 9-subject mean reflects the
\emph{population} reality including subjects where MI decoding
itself is intrinsically hard. We retain both as the honest pair
of summary numbers.

\paragraph*{When the agent commits within a trial}
Pooling decision-time stamps over all $9$ subjects, $5$ seeds and
$2$ training configs yields $N\!=\!3721$ commit decisions
(Fig.~\ref{fig:time_to_commit}). The agent uses approximately
$56\%$ of the available $4$-s trial before committing (median
$2.25$~s), confirming the operational claim that \textsc{Defer}
is being learned rather than collapsing to either end of the
trial. Commit accuracy rises monotonically with decision time
($0.48$ at $\le\!0.5$~s vs $\ge\!0.73$ beyond $1$~s), the expected
speed-accuracy trade-off; the small impulsive-commit mode
($\sim\!6\%$ of decisions firing at the first step) is the residual
EEGNet-confidence overrule the value head has not yet learned to
suppress.

\begin{figure}[!htbp]
\centering
\includegraphics[width=\columnwidth]{../figures/time_to_commit.png}
\caption{When the selective decoder commits within a trial, pooled
across 9 \bcia{} subjects $\times$ 5 seeds $\times$ 2 configs
($N\!=\!3721$ commit decisions); median decision time $2.25$~s
($\sim\!56\%$ of full trial length). (A) Commit-time distribution
is bimodal: $\sim\!6\%$ of decisions fire immediately at the first
step (impulsive-commit mode), the bulk ($\sim\!85\%$) cluster
between $1.25$--$3.0$~s as the value head accumulates evidence,
and very few fire in the $0.5$--$1.0$~s window. (B) Commit
accuracy by decision-time bin: early-impulse commits ($\le 0.5$~s)
drop to $0.48$, while every later bin ($\ge\!1$~s) is above
$0.73$, the expected speed-accuracy trade-off. Overall pooled
commit accuracy $0.751$.}
\label{fig:time_to_commit}
\end{figure}

\paragraph*{CVaR $\alpha$ sensitivity (cross-subject)}
The CVaR risk knob $\alpha$ collapses the learned $Z$-value
distribution to a scalar $Q$ by averaging over the left-tail
fraction $\alpha$, and is set at eval time without retraining
(distributional critic exposes all $\alpha$ from a single trained
agent). We sweep $\alpha \in \{0.10, 0.25, 0.50, 1.00\}$ on the
canonical 3-subject panel $\times$ 5 seeds with the same
distributional-CQL + CMDP $\epsilon\!=\!0.10$ training recipe
(Fig.~\ref{fig:cvar_alpha}; per-subject in
Appendix~\ref{app:cvar_alpha}). Mean commit accuracy is approximately
flat in $\alpha$ ($0.858$ at $\alpha\!=\!0.10$, peaks $0.882$ at
$\alpha\!=\!0.50$, $0.862$ at $\alpha\!=\!1.0$ --- range $2.4$pp),
but the safety-throughput trade-off moves monotonically: commit
rate rises $0.16\!\to\!0.29$ as $\alpha$ rises, and wrong-commit
rate doubles ($0.022\!\to\!0.040$). $\alpha\!=\!0.10$ is therefore
the safest setting (commits half as often, makes half the wrong
commits) while $\alpha\!=\!1.0$ trades safety for throughput at
$\sim\!4$pp higher \itr{} ($35.2$ vs $31.3$~b/m). The CMDP
constraint $\epsilon\!=\!0.10$ is satisfied for all $\alpha$
(cross-subject wrong-commit rates $0.022$--$0.040$ all well below
$0.10$). This is the first cross-subject ablation in our setup
showing that $\alpha$ functions as a designer-tunable knob rather
than being a vestigial parameter --- complementing the
within-subject \bcib{}~sub~4 result where all $\alpha$ produced
identical action sequences because the problem itself was too
easy.

\begin{figure}[!htbp]
\centering
\includegraphics[width=\columnwidth]{../figures/cvar_alpha_sensitivity.png}
\caption{CVaR $\alpha$ sensitivity on the canonical 3-subject
\bcia{} panel (3 subj $\times$ 5 seeds, eval-time sweep over a
single distributional-CQL+CMDP $\epsilon\!=\!0.10$ training).
(A) Commit accuracy (blue) and ITR (red) are approximately flat in
$\alpha$. (B) Wrong-commit rate (dark red, $0.022\!\to\!0.040$)
and commit rate (green, $0.16\!\to\!0.29$) both double from
$\alpha\!=\!0.10$ to $\alpha\!=\!1.00$. The CMDP constraint
($\epsilon\!=\!0.10$, dashed) is comfortably satisfied at every
$\alpha$. $\alpha$ is therefore an operating-point knob, not a
no-op.}
\label{fig:cvar_alpha}
\end{figure}

\paragraph*{Encoder-recipe ablation: published Song 2023 schedule}
A natural follow-up question is whether more compute on the
\emph{encoder pretraining} side closes the gap to the published
Conformer mandatory baseline. We rerun the Conformer encoder
pretraining under the recipe published in Song et al.\
2023~\cite{Song2023Conformer}: $250$ epochs (vs.\ our default $20$),
Adam $\beta_1\!=\!0.5$, $\beta_2\!=\!0.999$, and Shift-and-Rotate
augmentation with $N_s\!=\!8$ segments. The supervised encoder
val-accuracy lifts from $\sim\!0.78$ to $\sim\!0.87$ on subject 1,
matching the published mandatory baseline. The downstream
NeuroPolicy selective-decoder cross-subject result, however,
\emph{regresses} on both axes (Table~\ref{tab:sota}, ``Song 2023
recipe'' rows): scalar-\cql{} commit accuracy
$0.856\!\to\!0.795$ ($-6.1$pp); CVaR+CMDP
$0.876\!\to\!0.856$ ($-2.0$pp) with \itr{} dropping from
$37.0$~b/m to $32.7$~b/m. The likely mechanism: $250$ epochs of
subject-specific supervised pretraining hardens the encoder's
top-1 prediction at the cost of representational flexibility,
which the offline-RL agent then has less freedom to repurpose
through the value head. Stronger \emph{mandatory} encoder
pretraining is not a free upgrade for selective decoding ---
the two stages share an encoder and impose competing inductive
biases.
...
\begin{tabular}{@{}l r r r r@{}}
\toprule
Method & Comm.~acc & \itr{} (b/m) & Wrong & C-rate \\
\midrule
