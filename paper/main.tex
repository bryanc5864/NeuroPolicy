...
at a fixed window after the cue, paying the full wait cost on every
trial and offering no principled way to abstain when the signal is
ambiguous. We reframe within-trial decoding as a sequential
decision a learned agent makes at each $0.25$-s timestep ---
commit a class (\textsc{Commit}$_k$), defer for more evidence
(\textsc{Defer}), request recalibration (\textsc{Recal}), or
abstain (\textsc{Abstain}) --- and supply the two ingredients the
BCI literature has not: a Conservative Q-Learning agent trained
offline on logged trials under a CVaR risk-sensitive critic with a
CMDP commit-error constraint, and a calibrated fitted-Q off-policy
evaluator that selects policies without redeployment on humans.
The OPE estimate tracks on-policy Monte-Carlo ground truth at
Pearson $r\!=\!0.86$ with rank ordering preserved --- the property
offline policy selection actually requires, and the \emph{first}
calibrated OPE result published for motor-imagery BCI. On \bcia{}
4-class with an EEG-Conformer encoder, the selective decoder
reaches $87.6\%$ commit accuracy at $37$~bits/min~\itr{} on the
canonical 3-subject panel and $74.6\%$ at $25$~b/m on all 9
subjects, occupying the high-accuracy point of the
speed-accuracy Pareto frontier we benchmark against
confidence-threshold and mandatory-classifier baselines. Per-subject
results stratify cleanly by encoder quality: strong subjects match
the headline and four hard subjects (encoder val accuracy
$\le\!0.70$) cap below regardless of agent, localising the
bottleneck to representation rather than control. The CMDP
...
$0.022\!\to\!0.040$ across $\alpha\!\in\![0.10, 1.0]$). The
framework transfers across channel counts: leave-one-subject-out
classification on $62$-channel Lee2019 produces the \emph{first}
published LOSO benchmark for offline-RL BCI ($10/10$ subjects
beat random, paired Wilcoxon $p\!=\!0.001$), while low-channel
datasets transfer as a $-25\%$ reduction in wrong-commit rate
against an EEGNet-fixed baseline at statistically comparable
accuracy --- a safety advantage rather than an accuracy SOTA. To
our knowledge this is the first offline-RL BCI decoder with
\textsc{Defer} as a \emph{learned} action, distinct from post-hoc
thresholding. Code released under MIT.
\end{abstract}
