...
\begin{abstract}
Motor-imagery brain--computer interfaces (BCIs) usually pick a
single class label at a fixed window. We instead frame the
decoding problem as a sequence of decisions a learned agent must
make: at each timestep it can commit a class
(\textsc{Commit}$_k$), wait for more evidence
(\textsc{Defer}), request a recalibration trial
(\textsc{Recal}), or abstain (\textsc{Abstain}). Training such
an agent from logged trials requires offline reinforcement
learning, and \emph{evaluating} a candidate policy without
re-running it on a user requires off-policy evaluation (OPE) ---
both standard in clinical decision-making but absent from BCI.

We close that gap. First, we port calibrated fitted-Q evaluation
to BCI: across a 12-policy panel on BCI-IV-2b subject~4, our OPE
estimate tracks on-policy Monte-Carlo ground truth at Pearson
$r\!=\!0.860$ --- to our knowledge the first calibrated OPE
estimate published for BCI motor imagery. Second, under the
standard BCI-IV-2a 4-class evaluation protocol the agent reaches
$86.2\!\pm\!5.8\%$ commit accuracy at $34.2$~bits/min ITR, with
\textsc{Defer} as a \emph{learned} action rather than a tuned
threshold. Third, on 62-channel Lee2019 we run the first published
leave-one-subject-out classification benchmark for offline-RL BCI
($N\!=\!10$, $10/10$ subjects beat random, paired Wilcoxon
$p\!=\!0.001$); together with non-significant results on
3-channel and 22-channel datasets, this completes a clear
electrode-count $\to$ cross-subject-significance trend.
Supplementary contributions include a multi-method OPE benchmark
(FQE/PDIS/WIS/DR) showing WIS gives $3\times$ lower bias than FQE,
and the first offline-RL fine-tuning of the LaBraM EEG foundation
model. Code under MIT.
\end{abstract}
