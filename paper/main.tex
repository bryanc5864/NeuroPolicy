...
make: at each timestep it can commit a class, wait for more
evidence (\textsc{Defer}), request a calibration trial, or
abstain. Training such an agent from logged trials requires
offline reinforcement learning, and \emph{evaluating} a candidate
policy without re-running it on a user requires off-policy
evaluation (OPE) --- both standard in clinical decision-making but
absent from BCI.
