% Camera-ready — ICIST 2026, Submission #27, Accept (Oral).
...
\begin{abstract}
Motor-imagery brain--computer interfaces (BCIs) commit to a class
label at a fixed window after the cue: every trial pays the full
wait cost, and there is no principled way to abstain when the
signal is ambiguous. Our central contribution is to reframe
within-trial decoding as a sequential decision problem: at each
$0.25$-s step a learned agent commits to a class, defers for more
evidence, requests recalibration, or abstains. The agent is
trained fully offline from logged trials with Conservative
Q-Learning, extended with a risk-sensitive critic (conditional
value-at-risk, CVaR) and a constrained-MDP (CMDP) budget that
bounds the wrong-commit rate by design; candidate policies are
then selected \emph{without redeployment on humans} using a
calibrated fitted-Q off-policy evaluator --- the first calibrated
off-policy-evaluation (OPE) result for motor-imagery BCI (Pearson
$r\!=\!0.860$ against on-policy ground truth on a 12-policy
panel, rank ordering preserved). Because the agent is a
\emph{selective} decoder, it answers only when the evidence
warrants: on \bcia{} 4-class with an EEG-Conformer encoder it
commits on $27\%$ of trials and achieves $87.6\!\pm\!2.9\%$
accuracy on those commits at $37.0$~bits/min (canonical 3-subject
panel; $74.6\!\pm\!11.9\%$ at a $29\%$ commit rate over all 9
subjects), whereas a mandatory-commit EEGNet baseline answering
every trial reaches $81.5\%$ at $20.7$~b/m. The CMDP budget
($\epsilon\!=\!0.10$) is satisfied in every reported
configuration, and sweeping the CVaR level
$\alpha\!\in\![0.10,1.0]$ moves commit rate $0.16\!\to\!0.29$ and
wrong-commit rate $0.022\!\to\!0.040$ --- a safety--throughput
dial clinicians can specify in advance. On 62-channel Lee2019 we
report the first leave-one-subject-out benchmark for offline-RL
BCI ($10/10$ subjects beat random, $p\!=\!0.001$); against an
EEGNet-fixed baseline, accuracy is comparable ($p\!=\!0.43$)
while wrong-commit rate trends $25\%$ lower ($p\!=\!0.07$) --- a
safety advantage rather than an accuracy record. To our knowledge
this is the first offline-RL BCI decoder with \textsc{Defer} as a
learned action; with calibrated OPE it supplies the missing
infrastructure for clinical BCI deployments whose error cost is
bounded by design and whose policy choice is auditable offline.
...
% =========================================================================
We presented \nettype{}, an offline-RL framework with a
$\{\textsc{Commit}_k, \textsc{Defer}, \textsc{Recal},
\textsc{Abstain}\}$ action space, the first calibrated OPE
pipeline for BCI ($r\!=\!0.86$), and the first published Lee2019
LOSO benchmark for offline-RL BCI ($10/10$ wins, $p\!=\!0.001$;
$-25\%$ wrong-commit vs EEGNet-fixed), completing a channel-count
to LOSO-significance trend.
...
\textbf{Code} (MIT license): full pipeline + per-experiment
summary JSONs at
\url{https://github.com/bryanc5864/NeuroPolicy}; every
