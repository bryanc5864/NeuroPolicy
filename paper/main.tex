...
sequential decision a learned agent makes at each
$0.25$-s timestep --- commit a class
(\textsc{Commit}$_k$), defer for more evidence
(\textsc{Defer}), request recalibration (\textsc{Recal}), or
abstain (\textsc{Abstain}) --- and supply the two missing
ingredients: a Conservative Q-Learning agent trained offline on
logged trials under a CVaR risk-sensitive critic with a CMDP
commit-error constraint, and a calibrated fitted-Q off-policy
evaluator that lets us select policies without redeploying them
on humans.

The OPE estimate tracks on-policy Monte-Carlo ground truth at
Pearson $r\!=\!0.86$ with rank ordering preserved, the property
offline policy selection actually needs --- the first calibrated
OPE result published for BCI motor imagery. On \bcia{} 4-class
with an EEG-Conformer encoder, the selective decoder reaches
$87.6\%$ commit accuracy at $37$~bits/min~\itr{} on the canonical
$3$-subject panel and $74.6\%$ at $25$~b/m on the full $9$-subject
panel; per-subject results stratify by encoder quality, with
strong subjects matching the headline and four hard subjects
(encoder val accuracy $\le\!0.70$) capping below regardless of
agent, localising the bottleneck to representation rather than
control. The CMDP commit-error budget ($\epsilon\!=\!0.10$) is
satisfied on every reported configuration. The framework
transfers across channel counts: leave-one-subject-out classification
on $62$-channel Lee2019 delivers the first published LOSO
benchmark for offline-RL BCI ($10/10$ subjects beat random,
paired Wilcoxon $p\!=\!0.001$), while low-channel datasets
transfer as a safety advantage rather than an accuracy SOTA
($-25\%$ wrong-commit rate vs.\ an EEGNet-fixed baseline at
statistically comparable commit accuracy). Code released under
MIT.
\end{abstract}
