...
humans. The estimator tracks on-policy Monte-Carlo ground truth
at Pearson $r\!=\!0.86$ with rank ordering preserved, sufficient
to pick the best of a candidate set without an online trial ---
the \emph{first} calibrated OPE result published for motor-imagery
BCI. On \bcia{} 4-class with an EEG-Conformer encoder, the
selective decoder reaches $87.6\%$ commit accuracy at
$37$~bits/min \itr{} on the canonical 3-subject panel, exceeding
the mandatory EEGNet baseline ($0.82$ / $21$~b/m) on every axis;
extended to all 9 subjects it reaches $74.6\%$ at $25$~b/m. Per-subject
results stratify cleanly by encoder quality: strong subjects match
the headline, while four hard subjects (encoder val accuracy
$\le\!0.70$) cap below regardless of agent. The bottleneck is
...
users. Low-channel datasets transfer as a $-25\%$ reduction in
wrong-commit rate against an EEGNet-fixed baseline at
statistically comparable accuracy --- a safety advantage rather
than an accuracy SOTA. To our knowledge this is the first
